...
const GaugeChartComponent = () => {
  ...
  const drawGauge = (sPercentage, nsPercentage, selectedStatus) => {
    ...
    const svg = d3.select(gaugeRef.current).append("svg")
      ...
      .attr("preserveAspectRatio", "xMinYMin meet")// Adjust to meet centered scaling
      .style("width", "100%"); // Makes SVG responsive
  ...
  const drawHistogram = (propertyValues, selectedPropertyValue) => {
    ...
    const histSvg = d3.select(histogramRef.current).append("svg")
      .attr("viewBox", `0 0 ${histWidth + margin.left + margin.right} ${histHeight + margin.top + margin.bottom + 70}`)
  ...
  //updated chart depend on attempt that user select
  const updateCharts = () => {
  ...
  return (
    <div>
      <h1 className='text-capitalize'>Sold, On Sale and Property Score Distribution</h1>
      <p className='chart-desc'>
      This chart shows the percentage distribution of S and NS properties. Users can enter a property score to see where it falls within the distribution, providing a visual representation of its position in the histogram.
      </p>
      <select onChange={(e) => setSelectedAttempt(e.target.value)}>
        ...
        {salesPredictions.map((attempt) => (
          <option key={attempt.id} value={attempt.id}>
            Attempt {attempt.id} - Property Score: {attempt.predicted_result}
          </option>
        ))}
      </select>
